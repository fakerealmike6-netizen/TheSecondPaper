"""R4 extracted-bundle validation through the unchanged strict R1 guard.
...
from package_review_r4 import (EQUIVALENCE, MANIFEST, payload_files,
...
def verify_payload_r4(tree, kind):
    ...
    if mapping.get('schema') != 'stage1b-r4-payload-equivalence-dag-v1':
...
class ValidatorR4(Validator):
    def __init__(self, tree, output, kind, manifest_path=None):
        ...
        if kind == 'min':
            ...
            if value != {'schema_version': 'stage1b-r4-public-validation-v1'}:
    ...
    def closure_checks(self):
        matrix = read(input_path(self.tree, '03_REPAIR_CLOSURE_MATRIX.json'))
        findings = matrix.get('findings', [])
        self.check('nine_repair_closure', isinstance(findings, list)
                   and {r.get('id') for r in findings} == {'F%02d' % n for n in range(1, 10)}
                   and len(findings) == 9 and all(r.get('status') == 'FIXED_AND_TESTED' and r.get('code') and r.get('regression_tests') and r.get('callers') for r in findings),
                   finding_statuses={r.get('id'): r.get('status') for r in findings})
        self.check('bounded_retry_closure', matrix.get('E01', {}).get('status') == 'IMPLEMENTED_AND_OFFLINE_TESTED')
    ...
    def run(self):
        ...
        required += ['nine_repair_closure', 'bounded_retry_closure', 'repair_source_binding', 'external_acceptance_remains_pending']
        if self.kind == 'min':
        ...
        receipt = {'schema_version': 'stage1b-r4-portable-validation-v1',
...
def main():
    ...
    return ValidatorR4(args.tree, args.output, args.kind, args.manifest).run()
