...
def _key(value):
    if value is None:
        return None
    if not isinstance(value, str) or not value.endswith("|ETH"):
        raise ValueError("R3 context account must explicitly use |ETH")
    return value
...
def validate_document(document):
    ...
    for account in document.get("accounts", []):
        ...
        if key in accounts or key == BOUNDARY:
...
def _ports(flow, accounts, terminals):
    ...
    if role == "UNKNOWN_EXTERNAL_INCOMING":
        ...
        return BOUNDARY, receiver, "PRIOR_BOUNDARY_SOURCE_ONLY"
    ...
    if role == "BOUNDARY_OUTFLOW":
        ...
        return sender, BOUNDARY, "BOUNDARY_RESERVOIR"
    if receiver in accounts or receiver in terminals:
...
def build_context_model(document, *, remove_balance_information=False):
    ...
    model = Model(metadata={"model_type": "R3_ETH_CONTEXT_TIME_EXPANDED_LP",
        "schema_version": SCHEMA, "query_id": document.get("query_id"),
        ...
        "source_potential_raw": {"ETH": fmt(potential)},
        "scales_raw_per_solver_unit": {"ETH": fmt(potential)},
        ...
        "gaps": copy.deepcopy(document.get("gaps", [])),
    ...
    actual = {**initial, **{k: None for k in terminals}, BOUNDARY: None}
    last = {}
    ...
    def state(key, name, previous, delta, actual_balance, evidence, operation, kind="balance"):
        idx = model.var(name, "ETH", potential, F(0), cap(actual_balance), kind)
        row = {idx: F(1)}
    ...
    def operation(eid, sender, receiver, amount, fixed, evidence, role):
        idx = model.var(eid, "ETH", potential, fixed if fixed is not None else F(0),
    ...
    for tx in document.get("transactions", []):
        ...
        for flow in tx.get("flows", []):
            ...
            operation(flow["event_id"], sender, receiver, amount, fixed,
                      flow.get("evidence_ids", []), flow["role"])
    ...
    model.metadata.update({"known_initial_balances": sum(v is not None for v in initial.values()),
        ...
        "balance_status": "INFORMATION_RELAXED" if remove_balance_information else "PARTIAL_CONTEXT_WITH_EXPLICIT_GAPS" if document.get("gaps") or any(v is None for v in initial.values()) else "FULL_CONTEXT_WITHIN_DECLARED_MODEL_SCOPE"})
    return model
...
def target_completeness(document, events):
    ...
    terminals = set(document.get("objective_groups", {}))
    adjacent = {key: set() for key in set(accounts) | {BOUNDARY}}
    event_senders = {}
    ...
    relevant_gaps = [gap for gap in document.get("gaps", [])
                     if gap.get("account_id") is None or gap.get("account_id") in dependencies]
    unknown = [key for key in sorted(dependencies & set(accounts))
               if accounts[key].get("initial_actual_balance_raw") is None]
    return {"status": "PARTIAL_CONTEXT_WITH_EXPLICIT_GAPS" if relevant_gaps or unknown else "FULL_CONTEXT_WITHIN_DECLARED_MODEL_SCOPE",
...
def audit_context_witness(document, event_source_raw, *, remove_balance_information=False):
    ...
    terminals = set(document.get("objective_groups", {}))
    source = {k: F(0) for k in sorted(set(accounts) | terminals | {BOUNDARY})}
    actual = {k: None if a.get("initial_actual_balance_raw") is None else raw(a["initial_actual_balance_raw"])
              for k, a in accounts.items()}
    actual.update({k: None for k in terminals | {BOUNDARY}})
    ...
    if remaining + total_fee != seed_total:
...
def solve_context_interval(model, document, events, *, time_limit_seconds=60):
    import numpy as np
    ...
    for label, sign in (("lower", 1), ("upper", -1)):
        ...
        audit = audit_context_witness(document, witness, remove_balance_information=model.metadata["variant"] == "MATCHED_INFORMATION_RELAXED") if witness is not None else None
    ...
    return {"status": "OPTIMAL_EXACT_CERTIFIED" if passed else "UNRESOLVED", "asset": "ETH",
