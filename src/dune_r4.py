...
class ContextDuneR4(RevisionLive):
    def __init__(self, work, transport=None, *, clock=time.time, sleeper=time.sleep,
                 rng=random.random, deadline=None):
        ...
        self.reads = ReadRetryStore(retry_path(self.w), clock=clock, rng=rng)
    ...
    def require_gate(self):
        if self.transport is None: require_gate(self.w)
    ...
    def _preflight(self):
        ...
        if raw_risk(self.w) + DUNE_MAX > RAW_CAP: raise RuntimeError('Cumulative raw risk cap')
    ...
    def poll(self, folder):
        ...
        while True:
            ...
            return {'http_status': 200, 'state': state['state'], 'execution_id': state['execution_id'], 'observation_sequence': sequence,
                    'execution_cost_credits': state.get('execution_cost_credits'), 'cumulative': self.db.snapshot()['dune_credits']}
        ...
        while True:
            ...
            if claim['state'] == 'CACHE_HIT': body, receipt = claim['payload']['body'], claim['payload']['receipt']
                ...
                try: upper = self._reserve_page(state, claim, base, basis)
                    ...
                    self.reads.abandon_before_dispatch(claim['attempt_id'], 'BUDGET_OR_EVIDENCE_DEFERRED'); raise
            ...
            return {'export_status': state['export_status'], 'progress': validated, 'cache_reused': claim['state'] == 'CACHE_HIT',
                    'upper_not_actual': retained, 'cumulative': self.db.snapshot()['dune_credits']}
    ...
    def submit(self, sqlpath, label, kind='context', freeze_manifest=None, performance='medium', *, _retry=None):
        ...
        if kind != 'context' or performance not in ('small', 'medium'): raise ValueError('Only finite context Small/Medium work')
        sqlpath = self._inside(sqlpath); freeze_manifest = self._inside(freeze_manifest)
        verify_frozen_scope(freeze_manifest, self.w, sql_path=sqlpath)
        self._preflight()
        ...
        self.db.reserve_dune_job(job, label, '20', '0')
        ...
        state = {'logical_job_id': job, 'query_label': label, 'kind': kind, 'sql_sha256': digest, 'performance': performance,
            'reserved_execution': '20', 'reserved_export': '0', 'export_requests': 0, 'export_offsets': [],
    ...
    def settle(self, folder):
        ...
        if recovery:
            return {'settlement_status': 'EXPORTED_WITH_RETAINED_ATTEMPT_RISK' if exported else 'PARTIAL_WITH_RETAINED_ATTEMPT_RISK',
                    ...
                    'cumulative': self.db.snapshot()['dune_credits']}
        ...
        return {'settlement_status': status, 'known_execution_actual': str(actual), 'export_actual': None if exported else '0', 'cumulative': self.db.snapshot()['dune_credits']}
