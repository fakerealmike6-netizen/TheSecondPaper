...
class PhysicalFactRegistry:
    def __init__(self):self.groups={};self.aliases={};self._next=0
    ...
    def add(self,event,*,source=None,raw=None):
        ...
        if ids:
            ...
            group=self._next;self._next+=1;self.groups[group]={'versions':{},'aliases':set()}
        g=self.groups[group];g['aliases'].update(aliases)
        ...
        version={'facts':{k:d.get(k) for k in FACT_FIELDS},'event_id':d['event_id'],
                 ...
                 'raw':raw if raw is not None else (asdict(event) if is_dataclass(event) else dict(event))}
    ...
    def get(self,event_id):
        ...
        return None if conflicts else facts
    ...
    def snapshot(self):
        ...
        for g in self.groups.values():
            ...
            if fields:
                record={'reason':'PHYSICAL_FACT_CONFLICT','event_ids':ids,'fields':fields,
                        'physical_keys':sorted({physical_key(v['facts']|{'event_id':v['event_id']}) for v in versions}),
                        'versions':versions}
                record['conflict_id']=hashlib.sha256(json.dumps(record,sort_keys=True).encode()).hexdigest()
                conflicts.append(record);quarantine.extend(versions)
            else:events.append(facts)
        return {'events':sorted(events,key=lambda e:e['event_id']),
