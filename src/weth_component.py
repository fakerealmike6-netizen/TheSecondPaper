...
def verify_component(policy, tx, receipt, internal_response, *, call_trace=None, historical_code=None, source_attestation=None):
    ...
    if call_trace:
        ...
        if checks["unique_successful_call_frame"]:
            ...
            checks["deposit_bound_to_call_frame"] = len(frame_logs) == 1
    ...
    certified = base_valid and all(checks.values())
    status = "CERTIFIED_LOCAL_WETH_DEPOSIT" if certified else "OBSERVED_LOCAL_DEPOSIT_PATTERN_WITH_GAPS" if base_valid else "COMPONENT_EVIDENCE_INCONSISTENT"
    ...
    return {"status": status, "checks": checks, "gaps": sorted(set(gaps)), "runtime_code_sha256": runtime_sha, "source_sha256": attestation.get("source_sha256"), "native_locator": native_locator, "semantic_unit": unit, "boundary_comparison": {"model_rule": "1:1 source amount conversion on isolated deposit", "input_source_range_raw": ["0", str(amount)], "output_source_range_raw": ["0", str(amount)], "joint_relation": "x_native_input = x_weth_credit; these ports do not represent two independent source injections", "proof_scope": "single input / single output only; not arbitrary multioutput DSU equivalence", "real_component_use_permitted": certified}}
...
def main():
    ...
    result = verify_component(read(args.policy)["weth_component"], payloads["transaction"], payloads["receipt"], payloads["internal"], call_trace=read(args.call_trace), historical_code=read(args.historical_code), source_attestation=read(args.source_attestation))
