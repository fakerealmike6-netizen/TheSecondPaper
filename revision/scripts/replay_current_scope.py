...
import sys,json,time,hashlib,socket,argparse,os,uuid
...
@contextmanager
def offline_replay_lock(work,names):
 lock=Path(work)/'private/network_worker.lock'
 if lock.exists():raise RuntimeError('Existing writer lock; preserve it and wait for the normal safe point')
 payload=json.dumps({'pid':os.getpid(),'kind':'OFFLINE_QUERY_REPLAY','query':names[0] if len(names)==1 else 'SHARED',
  'query_names':names,'token':uuid.uuid4().hex,'started_unix_seconds':time.time(),
  'authorization_id':'STAGE1D_WINDOW_ROLE_SEMANTIC_CLOSURE_V1','online_clock_increment':0},sort_keys=True).encode('utf8')
 # Exclusive creation also closes the race after the explicit existence check.
 with lock.open('xb') as handle:handle.write(payload)
 try:
  Runtime().require_gate(work)
  yield
 finally:
  if lock.read_bytes()!=payload:raise RuntimeError('Worker lock changed; preserve it for explicit recovery')
  lock.unlink()

parser=argparse.ArgumentParser()
parser.add_argument('--query',action='append',choices=('txphish_src001','txphish_src002','xscam_src001','lifi_src001'),
 help='Repeat to replay only these queries, in the supplied order, with one shared cache/label/semantic initialization.')
args=parser.parse_args()
names=args.query or ['txphish_src001','txphish_src002','xscam_src001','lifi_src001']
if len(names)!=len(set(names)):parser.error('Each query may appear only once in an offline replay invocation')
with offline_replay_lock(C,names):
 ...
 result={'authorization_id':batch['authorization_id'],'batch_freeze_sha256':sha(active_batch_path(C)),
  ...
  'queries':list(prior['queries']) if prior else [],'new_external_requests':0}
 ...
 for name in names:
  ...
  c=Collector(provider,labels,limits,semantic_resolver=resolver).run(scope,Event(**q['seed_event'])).to_dict()
  ...
  # A later query failure must retain the other query's last saved disposition.
  result['queries']=[prior_row for prior_row in result['queries'] if prior_row['query_name']!=name]
