...
def derive_query(query,collection,collection_ref,freeze_ref):
    ...
    for e in all_events:
        ...
        # Collector's candidate envelope adds this explicit non-context flag;
        # retain its original bytes in the demand, while Event owns only facts.
        if e.get('context_only',False) is not False:raise ValueError('Context-only event cannot authorize current forward demand')
        event=Event(**{k:v for k,v in e.items() if k!='context_only'});links=[]
