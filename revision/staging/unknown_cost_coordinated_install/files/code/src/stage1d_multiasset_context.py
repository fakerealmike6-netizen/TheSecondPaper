...
def required_windows(query, collection, ledger_events=(), label_snapshot=None, *, prior_rows=()):
    ...
    prior_rows, deferred_prior = current_prior_rows(collection, list(rows.values()), prior_rows)
    ...
    plan = {'schema_version': SCHEMA, 'query_id': query['query_id'], 'name': query['name'],
    ...
    planned = project_plan(query, collection, plan)
    if deferred_prior:
