...
def prepare(work, boundary_path, query_name, output, *, maximum=20):
    ...
    cost_guard = validate_candidate_entry(work, query, entry)
    ...
    if cost_guard['status'] != 'NOT_APPLICABLE_UNCHANGED':
        plan['cost_candidate_request_guard'] = cost_guard
...
def acquire(work, plan_path, access, *, clock=time.time, maximum_pages=100, maximum_binding_calls=2000,
    ...
    cost_guard = validate_candidate_entry(work, query, entry)
    if cost_guard['status'] != 'NOT_APPLICABLE_UNCHANGED':
        if plan.get('cost_candidate_request_guard') != cost_guard:
            raise ValueError('Candidate plan has a stale cost policy/state request guard')
