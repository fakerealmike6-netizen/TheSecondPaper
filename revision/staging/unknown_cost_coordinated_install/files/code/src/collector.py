...
from stage1d_gap_sequence import (GapAccumulator, serialize_gaps, overlay_gaps,
                                  any_gap_reason, iter_gaps_with_reasons)
...
class FetchResult:
    ...
    def to_dict(self):
        data = asdict(replace(self, gaps=[]))
        data['gaps'] = serialize_gaps(self.gaps)
        return data
...
class CollectionResult:
    ...
    cost_boundary_policy: dict = field(default_factory=dict)
    cost_boundary_decisions: list[dict] = field(default_factory=list)

    def to_dict(self):
        data = asdict(replace(self, gaps=[]))
        data['gaps'] = serialize_gaps(self.gaps)
        if not self.cost_boundary_policy:
            data.pop('cost_boundary_policy')
            data.pop('cost_boundary_decisions')
        return data
    ...
    def write(self, path):
        ...
        temp = path.with_name(path.name + '.' + uuid.uuid4().hex + '.gap-write.tmp')
        try:
...
class Collector:
    ...
    """
    def __init__(self, provider, label_resolver, limits=None, checkpoint_path=None, *, semantic_resolver=None, cost_boundary_resolver=None):
        self.provider = provider
        self.label_resolver = label_resolver
        self.limits = limits or Limits()
        self.checkpoint_path = Path(checkpoint_path) if checkpoint_path else None
        self.semantic_resolver = semantic_resolver
        self.cost_boundary_resolver = (cost_boundary_resolver if cost_boundary_resolver is not None
                                       else getattr(label_resolver, 'cost_boundary_resolver', None))
    ...
    def run(self, scope, seed):
        ...
        processed, stops, pending, coverage, membership = [], [], [], [], []
        gaps = GapAccumulator()
        semantic_units, semantic_membership = {}, []
        cost_resolver = self.cost_boundary_resolver
        cost_policy = cost_resolver.policy_for_scope(scope) if cost_resolver is not None else None
        cost_decisions = []
        cost_identity = getattr(cost_resolver, 'identity', None) if cost_policy else None
        if cost_policy:
            if type(cost_policy.get('enabled')) is not bool or not cost_identity or not cost_policy.get('policy_sha256'):
                raise ValueError('Active cost resolver requires immutable policy and evidence identity')
            from stage1d_unknown_cost_boundary import validate_decision
        ...
        if self.checkpoint_path and self.checkpoint_path.exists():
            ...
            if prior.get('cost_boundary_resolver_sha256') != cost_identity:
                raise ValueError('checkpoint belongs to another cost policy/evidence catalogue')
        ...
        def checkpoint():
            if self.checkpoint_path:
                ...
                if cost_policy:
                    payload.update(cost_boundary_resolver_sha256=cost_identity,
                        cost_boundary_policy=cost_policy,cost_boundary_decisions=cost_decisions)
        ...
        while heap:
            ...
            decision = None
            if cost_policy:
                decision = cost_resolver.resolve(state, scope, identity)
                validate_decision(state, scope, decision, cost_policy['policy_sha256'])
                if not cost_policy['enabled'] and decision['action'] != 'BYPASS':
                    raise ValueError('Disabled cost policy permits only explicit BYPASS decisions')
                if decision.get('base_identity', identity) != identity:
                    raise ValueError('Cost decision may not change base identity')
                base['cost_boundary'] = decision
                cost_decisions.append(decision)
            processed.append(base)
            if identity.get('branch_action') == 'USER_REQUESTED_BRANCH_HOLD':
                # Preserve arrival/source state and ordinary-account facts. This
                # is an unfinished user-directed branch, never a service target.
                hold = {**base, 'reason': 'USER_REQUESTED_BRANCH_HOLD',
                        'entry_event_id': state.arrival.event_id}
                stops.append(hold)
                pending.append(hold)
                gaps.append({'reason': 'USER_REQUESTED_BRANCH_HOLD',
                             'address': state.address, 'query_id': state.query_id,
                             'arrival_event_id': state.arrival.event_id,
                             'task_boundary_ids': identity.get('task_boundary_ids', []),
                             'task_boundary_authority_ids': identity.get('task_boundary_authority_ids', []),
                             'resume_condition': 'EXPLICIT_USER_RESUME',
            ...
            if state.depth >= scope.max_depth:
                ...
                checkpoint()
                continue
            if decision and decision['action'] in ('STOP', 'PENDING'):
                # A stopped ordinary interval is not a holder-wide semantic
                # exemption. Only physical legs already certified in the finite
                # catalogue may be offered to the existing exact state resolver.
                # Prior user/role/component/depth boundaries have already won.
                finite = []
                if self.semantic_resolver is not None:
                    try:
                        admitted = getattr(self.semantic_resolver, 'by_holder', {}).get((state.address, state.asset), [])
                        finite = self.semantic_resolver.resolve(state,
                            [Event(**u['native_event']) for u in admitted], scope, identity)
                        for item in finite:
                            if 'gap' in item:
                                gaps.append(dict(item['gap'], address=state.address))
                            elif 'unit' in item:
                                registry.add(item['native_event'])
                        snapshot = registry.snapshot(include_events=False)
                        if snapshot['conflicts']:
                            fact_conflicts = snapshot['conflicts']; quarantined = snapshot['quarantined_versions']
                        ...
                        interrupted = CONFLICT_STATUS; pending.append({**base, 'reason': CONFLICT_STATUS}); break
                    units = [item for item in finite if 'unit' in item]
                    if units:
                        base['finite_semantic_continuation'] = {
                            'schema_version': 'stage1d-cost-finite-semantic-continuation-v1',
                            'state_key': decision['state_key'], 'decision_sha256': decision['decision_sha256'],
                            'semantic_resolver_sha256': self.semantic_resolver.identity,
                            'unit_ids': sorted({item['unit']['unit_id'] for item in units}),
                            'ordinary_discovery_allowed': False, 'final_ledger_complete_claimed': False}
                    for item in units:
                        unit = item['unit']; semantic_units[unit['unit_id']] = unit
                        if item['membership'] not in semantic_membership:
                            semantic_membership.append(item['membership'])
                        native = item['native_event']; candidates[native.event_id] = native
                        if item['replace_native_recipient_push']:
                            membership.append({'event_id': native.event_id,
                                'arrival_event_id': state.arrival.event_id, 'depth': state.depth + 1})
                        push(item['arrival'], state.depth + 1)
                boundary = {**base, 'reason': decision['reason'], 'entry_event_id': state.arrival.event_id}
                if decision['action'] == 'STOP':
                    stops.append(boundary)
                else:
                    pending.append(boundary)
                    gaps.append({'reason': decision['reason'], 'address': state.address,
                                 'arrival_event_id': state.arrival.event_id,
                                 'cost_state_key': decision['state_key'],
                                 'deferred_branch_only': True})
                # No interval fetch: the entering fact/source arrival stays in
                # the graph; other independently reached states still expand.
                if len(candidates) > self.limits.max_events:
            ...
            gaps.extend(overlay_gaps(result.gaps, {"address": state.address, "arrival_event_id": state.arrival.event_id}))
            ...
            fact_conflicts += [g for g in iter_gaps_with_reasons(result.gaps, ('PHYSICAL_FACT_CONFLICT','DUNE_EVENT_IDENTITY_CONFLICT')) if g not in fact_conflicts]
            if fact_conflicts:
        ...
        if interrupted:
            status = interrupted
        elif any_gap_reason(gaps, substring="BUDGET"):
            status = "INCOMPLETE_BUDGET_LIMIT"
        elif pending and all(p.get("reason") == "USER_REQUESTED_BRANCH_HOLD" for p in pending):
            status = "INCOMPLETE_USER_REQUESTED_BRANCH_HOLD"
        elif pending:
            status = "INCOMPLETE_PROVIDER_OR_DATA_GAP"
        elif any_gap_reason(gaps, ("ORDER_UNRESOLVED", "EVENT_IDENTITY_UNRESOLVED", "INTERNAL_STATUS_UNRESOLVED",
                "SEMANTIC_OPERATION_ORDER_UNRESOLVED","COMPONENT_INSTANCE_EVIDENCE_REQUIRED","SEMANTIC_INTEGRATION_GATE_CLOSED")):
            status = "COMPLETED_WITH_RECORDED_DATA_GAPS"
        else:
            status = ("COMPLETED_WITH_DECLARED_COST_BOUNDARIES"
                      if any(d['action'] == 'STOP' for d in cost_decisions)
                      else "COMPLETED_WITHIN_DECLARED_SCOPE")
        invalidated={}
        if fact_conflicts:
            ...
            if cost_policy:invalidated.update(cost_boundary_decisions=cost_decisions)
            candidates={};context={};processed=[];stops=[];membership=[]
            semantic_units={};semantic_membership=[]
            cost_decisions=[]
            pending=[{k:v for k,v in row.items() if k!='cost_boundary'} for row in pending]
        ...
        if self.semantic_resolver is not None:
            stable.update(semantic_units=semantic_units,semantic_membership=semantic_membership,
                semantic_resolver_sha256=self.semantic_resolver.identity)
        if cost_policy:
            stable.update(cost_boundary_policy=cost_policy,cost_boundary_decisions=cost_decisions,
                          cost_boundary_resolver_sha256=cost_identity)
        ...
        if cost_policy:
            result.cost_boundary_policy=dict(cost_policy)
            result.cost_boundary_decisions=cost_decisions
            result.metrics.update(cost_boundary_resolver_sha256=cost_identity,
                cost_boundary_state_count=sum(d['action']=='STOP' for d in cost_decisions),
                cost_boundary_pending_state_count=sum(d['action']=='PENDING' for d in cost_decisions),
                has_cost_boundary=any(d['action']=='STOP' for d in cost_decisions),
                has_pending_identity_or_type=any(d['action']=='PENDING' for d in cost_decisions),
                scope_complete=False if any(d['action'] in ('STOP','PENDING') for d in cost_decisions) else not pending)
        return result
