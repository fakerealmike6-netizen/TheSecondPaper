...
def register_document(tree,batch_dir,query_id,document,scope,label_snapshot,context_evidence,
    ...
    if acquisition_status in {'ACQUISITION_PARTIAL','PARTIAL_FULL_SCOPE'}:
        if context_status=='FULL_CONTEXT_WITHIN_DECLARED_MODEL_SCOPE':
            ...
            if not context_evidence.get('unresolved_frontier') and not gap_count(context_evidence.get('gaps', [])):raise ValueError('Partial acquisition must retain its missing frontier or gaps')
        from stage1d_gap_sequence import gap_count
        if not doc.get('assumptions') and not gap_count(doc.get('gaps', [])):raise ValueError('Partial model must state its limitations')
