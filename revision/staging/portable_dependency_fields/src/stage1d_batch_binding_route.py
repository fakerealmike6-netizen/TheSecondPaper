...
def import_completed(work, preparation_path, job_states):
    ...
    for need in manifest['need_rectangles']:
        ...
        record = {'acquisition_adapter': VERSION, 'provider': 'CLASSIC_BIGQUERY', 'evidence_id': key,
            ...
            'native_decision_path': h.dep(work, folder / 'proof.json')['path'], 'native_decision_sha256': h.sha(folder / 'proof.json'),
            'events_path': h.dep(work, folder / 'events.json')['path'], 'events_sha256': h.sha(folder / 'events.json'),
    ...
    return {'status': 'NATIVE_BATCH_BINDING_COMPLETE' if result['native_complete'] else 'BATCH_BINDING_PARTIAL_WITH_EXPLICIT_GAPS',
            ...
            'fee_gaps': result['validated']['fee_gaps'], 'proof': h.dep(work, folder / 'proof.json'),
...
def verify_interval_record(work, record, *, memo=None):
    ...
    key = ('BATCH_BINDING_PROOF', record['native_decision_path'], record['native_decision_sha256'])
    if memo is None or key not in memo:
        proof = h.read(transfers.checked(work, {'path': key[1], 'sha256': key[2]}))
        if proof['version'] != VERSION or h.digest(proof) != record['evidence_id']:
        ...
        value = (proof, h.read(prep), result)
        if memo is not None:
            memo[key] = value
    proof, manifest, result = memo[key] if memo is not None else value
    matches = [n for n in manifest['need_rectangles'] if record['addresses'] == [n['address']]
               and all(record[k] == n[k] for k in FIELDS)]
    if len(matches) != 1 or record.get('evidence_id') != h.digest(proof):
