...
def required_windows(query, collection, ledger_events=(), label_snapshot=None, *, prior_rows=()):
    ...
    modeled = set()
    facts = candidates + [_event(x) for x in collection.get('context_events', [])] + [_event(x) for x in ledger_events]
    for e in candidates:
        a = asset(e['asset'])
...
def build_document(query, collection, ledger_events, balances=None, block_headers=None,
    ...
    events = [_event(x) for x in material.get('events', [])]
    ...
    plan = required_windows(query, collection, events, label_snapshot, prior_rows=(context_plan or {}).get('rows', []))
    seed, candidates = _seed(query, collection)
    headers = _headers(block_headers or {})
    native_events = [e for e in candidates + [_event(x) for x in collection.get('context_events', [])] + events if asset(e['asset']) == ETH]
    ...
    for gap in collection.get('gaps',[]):
        doc['gaps'].append({'type':'CANDIDATE_ACQUISITION_GAP','account_id':None,'original_gap':copy.deepcopy(gap)})
    if collection.get('unresolved_frontier') or str(collection.get('status','')).startswith('INCOMPLETE'):
        doc['gaps'].append({'type':'CANDIDATE_COLLECTION_INCOMPLETE','account_id':None,'collection_status':collection.get('status')})
