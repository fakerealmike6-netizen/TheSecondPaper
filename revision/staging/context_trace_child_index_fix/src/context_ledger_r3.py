...
def normalize_rows(rows):
    ...
    # Index only merged physical traces; preserve their original insertion order.
    # A root is never a child. Missing paths were already excluded above.
    children_by_parent = defaultdict(list)
    for child in traces.values():
        path = child["trace_address"]
        if path:
            children_by_parent[(child["tx_hash"], path[:-1])].append(child)
    for key, row in traces.items():
        if row["subtraces"] is not None:
            children = children_by_parent.get((row["tx_hash"], row["trace_address"]), ())
