...
from stage1d_multiasset_context import ETH, WETH, NATIVE, CONTRACT, asset
...
def validate_current(query, collection, labels):
    ...
    return {'scope': scope.freeze_dict(), 'scope_hash': scope.scope_hash,
...
def _project(rows, plan):
    ...
    def touched(original, event):
        ...
        return any(asset(event.get('asset', NATIVE)) == asset(row.get('asset', NATIVE))
...
def _native_receipt_reuse(facts, plan, material, mandatory):
    ...
    for original in facts:
        ...
        if asset(event.get('asset', NATIVE)) == ETH and event.get('tx_hash'):
...
def requirements(query, collection, labels, material=None):
    ...
    facts = collection.get('candidate_events', []) + collection.get('context_events', []) + selected
    ...
    for original in facts:
        event = _event(original)
        if (asset(event.get('asset', NATIVE)) == ETH and event.get('kind') == 'top'
    ...
    mandatory_receipts.update(_event(r)['tx_hash'] for r in facts
                              if asset(_event(r).get('asset', NATIVE)) == WETH and _event(r).get('tx_hash'))
...
def assemble(query, collection, labels, material, *, binding=None):
    ...
    facts = {'events': needed['selected_events'], 'coverage': copy.deepcopy(material.get('coverage', [])),
    ...
    if incomplete_logs:
        ...
        result = {'model_input': None, 'context_plan': needed['context_plan'],
            ...
            'blocked_observed_material': {'collection':copy.deepcopy(collection), 'material':copy.deepcopy(material)}}
    else:
    ...
    if missing:
        ...
        if result.get('model_input'):
            # Missing current requirements must accompany the actual common
            # input, not only its outside acquisition receipt.
            doc_gaps = result['model_input'].setdefault('gaps', [])
            if doc_gaps is not result['evidence_gaps']:
                doc_gaps.extend(copy.deepcopy(gaps))
