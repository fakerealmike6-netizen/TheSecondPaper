...
from stage1d_gap_sequence import as_gap_sequence, concat_gaps, exclude_gaps
...
def _normalization_gap_suffix(original_gaps, matching):
    """Lazy original-prefix exclusion; every ordered suffix occurrence survives.

    Repeated references to one immutable base are filtered once per fetch, then
    concatenated in original record order. The original prefix alone determines
    Python membership, never earlier suffix entries. No logical list is expanded.
    """
    filtered, pieces = {}, []
    for record in matching:
        source = record.get('_normalization_gap_sequence', record.get('normalization_gaps', ()))
        key = id(source)
        if key not in filtered:
            filtered[key] = exclude_gaps(source, original_gaps)
        pieces.append(filtered[key])
    return concat_gaps(*pieces)


def _record_gap_conflicts(record):
    if '_normalization_gap_conflicts' in record:
        return record['_normalization_gap_conflicts']
    return [g for g in record.get('normalization_gaps', ())
...
class CachedIntervals:
    ...
    def __init__(self, work):
        ...
        for p in sorted((self.work/'derived/stage1d/intervals').glob('*.coverage.json')):
            ...
            if r.get('acquisition_adapter') == 'stage1d-transfers-acquisition-v1':
                ...
                verified = verify_interval_record(self.work, r, memo=content_memo)
                # verify_interval_record already checked exact complete/gap
                # equality against this immutable proof. Share that verified
                # base only after verification, never by an untrusted hash key.
                r['normalization_gaps'] = verified['gaps']
            ...
            base_gaps = r.get('normalization_gaps', [])
            gap_key = id(base_gaps)
            if gap_key not in gap_memo:
                gap_memo[gap_key] = (base_gaps, as_gap_sequence(base_gaps),
                    tuple(g for g in base_gaps if g.get('reason') in
                          ('PHYSICAL_FACT_CONFLICT', 'DUNE_EVENT_IDENTITY_CONFLICT')))
            # Keep the plain shared base for existing internal checks, and its
            # immutable codec sequence for lazy per-arrival aggregation.
            _, r['_normalization_gap_sequence'], r['_normalization_gap_conflicts'] = gap_memo[gap_key]
    ...
    def fetch_interval(self,address,asset,start_block,end_block,*,start_time,end_time,global_end_time):
        ...
        explicit_conflicts=[g for r in matching for g in _record_gap_conflicts(r)]
        ...
        if missing:gaps = concat_gaps(gaps, _normalization_gap_suffix(gaps, matching))
...
class Labels:
    def __init__(self,work):
        ...
        self.cost_boundary_resolver=None
        if (self.work/'private/stage1d_roles/UNKNOWN_COST_POLICY.json').exists() or (self.work/'private/stage1d_roles/UNKNOWN_COST_CURRENT.json').exists():
            from stage1d_unknown_cost_registry import Registry
            self.cost_boundary_resolver=Registry(self.work,labels=self)
...
def replay(work,query):
    ...
    return collection.to_dict(),provider.pending
...
def acquire_pending(work,query,pending,collection_path):
    ...
    validate_candidate_entry(work,query,{'query_name':query['name'],'query_id':query['query_id'],
        'scope_hash':Scope.from_policy(query).scope_hash,
        'collection_path':collection_path.relative_to(work).as_posix(),
        'collection_sha256':sha(collection_path),'pending_intervals':pending})
