...
CURATED_STOP_BASIS='CURATED_EXACT_CHAIN_ADDRESS_PROTOCOL_SCOPE_STOP_V1'
...
def validate_certificate(work,certificate):
 ...
 if c.get('decision_basis')==CURATED_STOP_BASIS:
  from stage1d_curated_scope_stop import validate_scope_stop
  return validate_scope_stop(root,c)
...
class TechnicalRoles:
 ...
 def resolve(self,state,identity):
  ...
  def applies(c):
   ...
   if c.get('decision_basis')==CURATED_STOP_BASIS:
    from stage1d_curated_scope_stop import applies_to_state
    return applies_to_state(c,state)
  ...
  direct=[c for c in matches if c.get('decision_basis') in (STOP_BASIS,CURATED_STOP_BASIS)]
  if direct:
   curated=any(c.get('decision_basis')==CURATED_STOP_BASIS for c in direct)
   return {**result,'kind':'UNSUPPORTED_PROTOCOL','actor':None,'custodial_actor_status':'NOT_ESTABLISHED',
    'technical_role_status':'CURATED_SOURCE_SUPPORTED_SCOPE_BOUNDARY' if curated else 'OFFICIAL_SOURCE_SUPPORTED_SCOPE_BOUNDARY','branch_action':'UNSUPPORTED_PROTOCOL_STOP',
    'role_certificate_ids':[m['certificate_id'] for m in matches],'stop_decision_basis':CURATED_STOP_BASIS if curated else STOP_BASIS,
