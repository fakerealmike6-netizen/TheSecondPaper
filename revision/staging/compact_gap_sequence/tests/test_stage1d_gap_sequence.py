...
class CollectorGapIntegrationTests(unittest.TestCase):
    ...
    def test_complete_legacy_collector_value_order_and_status_equal(self):
        spec=importlib.util.spec_from_file_location('synthetic_original_collector',C/'src/collector.py');old=importlib.util.module_from_spec(spec);sys.modules[spec.name]=old;spec.loader.exec_module(old)
        seed,provider,labels,scope=self.fixture()
        before=asdict(old.Collector(provider(),labels).run(old.Scope(**asdict(scope)),old.Event(**asdict(seed))))
    ...
    def test_write_hash_and_restart_preserve_compact_gaps(self):
        ...
        with tempfile.TemporaryDirectory(dir=S/'tmp') as tmp:
