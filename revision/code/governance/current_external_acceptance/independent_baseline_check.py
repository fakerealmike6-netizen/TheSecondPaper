...
def main():
 ...
 for row in rows:
  doc=read(ROOT/row['observed_path']);got=independent(doc);kind=row.get('kind','controlled');dest=ROOT/'results'/kind/'samples'/row['sample_id'].replace('/','_').replace(':','_');saved=read(dest/'METHOD_RESULTS.json')
 ...
 (R/'independent_baseline_results.json').write_text(json.dumps(report,indent=2));print({k:v for k,v in report.items() if k!='rows'});print([x for x in result if x['kind']=='real'])
