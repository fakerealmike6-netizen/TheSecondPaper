"""Guarded private R4 reproduction from source responses through exact LP.
...
from validate_review_bundle_r4 import checked_config_r4, PILOTS
...
def check_weth_input_paths(tree, manifest_path):
    ...
    if manifest.get('schema_version') != 'stage1b-r4-weth-replay-v1' or set(manifest) != {'schema_version','r3_inputs','dune_trace_manifest'}:
    ...
    for role in ('r3_inputs','dune_trace_manifest'):
        ...
        if not path.is_relative_to(tree) or hashlib.sha256(path.read_bytes()).hexdigest() != entry['sha256']:
            raise ValueError('WETH input manifest hash or length mismatch')
        nested=read(path)
        for item in nested.get('files',{}).values():
            payload=input_path(path.parent,item['path'])
            if not payload.is_relative_to(tree) or payload.stat().st_size != item['bytes'] or hashlib.sha256(payload.read_bytes()).hexdigest() != item['sha256']:
                raise ValueError('WETH nested evidence binding mismatch')
    return manifest


def replay_review_r4(tree, config, output):
    tree, output = Path(tree).resolve(), Path(output).resolve()
    checked_config_r4(tree, config)
    context_manifest = input_path(tree, config['context_manifest'])
    check_context_input_paths(tree, context_manifest)
    output.mkdir(parents=True, exist_ok=True)
    baseline_comparisons, context_comparisons, proofs, r3_comparisons = [], [], [], []
    ...
    for case in config['expected_context']:
        ...
        original_case = next(row for row in config['r3_baseline'] if row['name'] == case['name'])
        original_ledger = read(input_path(tree, original_case['ledger']))
        original_amounts = read(input_path(tree, original_case['amounts']))
        expected_amounts = read(input_path(tree, case['amounts']))
        actual_amounts = run_context_document(document)
        r3_comparisons.append({'name': case['name'],
            'same_physical_input_model_equal': original_ledger['model_input'] == document,
            'amount_endpoints_and_status_equal': semantic_amounts(original_amounts) == semantic_amounts(actual_amounts),
            'full_ledger_artifact_equal': semantic_ledger(original_ledger) == semantic_ledger(rebuilt),
            'comparison_is_observation_not_forced_expected_amount': True,
            'original_ledger_sha256': hashlib.sha256(input_path(tree, original_case['ledger']).read_bytes()).hexdigest(),
            'original_amount_sha256': hashlib.sha256(input_path(tree, original_case['amounts']).read_bytes()).hexdigest()})
    ...
    result = {'schema_version': 'stage1b-r4-private-replay-comparison-v1',
              ...
              'r3_baseline_comparisons': r3_comparisons,
    ...
    write(output / 'R4_REPLAY_COMPARISON.json', result)
    return result
...
def main():
    ...
    result = replay_review_r4(tree, read(input_path(tree, args.config)), args.output)
