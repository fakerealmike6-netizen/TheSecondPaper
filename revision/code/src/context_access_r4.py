...
def runtime_budget_ledger(work, runtime=None):
    """Select a separately authorized ledger; a runtime cap is never a grant."""
    factory = getattr(runtime, 'ledger_factory', RevisionLedger)
    if not callable(factory): raise ValueError('Runtime ledger factory must be callable')
    ledger = factory(db_path(work))
    cap = getattr(runtime, 'cumulative_cap', None)
    if cap is not None:
        cap = Decimal(str(cap))
        if not cap.is_finite() or cap <= 0: raise ValueError('Positive finite runtime cumulative cap required')
        if Decimal(str(ledger.snapshot()['dune_credits']['cap'])) != cap:
            raise RuntimeError('Runtime cumulative cap does not match authorized ledger')
        warning = getattr(runtime, 'warning_threshold', None)
        if warning is not None:
            warning = Decimal(str(warning))
            if not warning.is_finite() or not 0 < warning <= cap:
                raise ValueError('Runtime warning threshold must be within cumulative cap')
    return ledger


def runtime_budget_warning(work, runtime, snapshot):
    """Deliver an optional reminder; its threshold is never a spending gate."""
    hook = getattr(runtime, 'warn_budget', None)
    threshold = getattr(runtime, 'warning_threshold', snapshot['dune_credits'].get('warning_threshold'))
    if hook is None or threshold is None: return None
    if not callable(hook): raise ValueError('Runtime warning hook must be callable')
    if Decimal(snapshot['dune_credits']['cumulative_risk']) < Decimal(str(threshold)): return None
    try:
        return hook(Path(work), snapshot)
    except Exception as exc:
        # The accessor retains this diagnostic without blocking a reserved read.
        return {'warning_delivery_failed': type(exc).__name__, 'warning_requires_pause': False}
...
class RpcAccess(R3RpcAccess):
    def __init__(self, work, transport=None, *, clock=time.time, monotonic=time.monotonic, sleep=time.sleep, rng=random.random, runtime=None):
        self.w = Path(work).resolve()
        if not db_path(self.w).exists(): raise RuntimeError('R4 budget migration required')
        self.ledger = runtime_budget_ledger(self.w, runtime); self.transport = transport
        self.clock, self.monotonic, self.sleep = clock, monotonic, sleep
        self.store = getattr(runtime,'retry_store',ReadRetryStore)(retry_path(self.w), clock=clock, rng=rng)
        self.runtime = runtime

    def _transport(self, requests):
        from stage1d_recovery_policy import effective
        recovery=effective(self.w) if self.runtime is not None else None
        if recovery and self.transport is None:
            import requests as http_requests
            key=os.environ.get('ALCHEMY_API_KEY')
            if not key:raise RuntimeError('Configured Alchemy credential absent')
            last_path=self.w/'private/stage1d_rpc_last_dispatch.json'
            last=read(last_path) if last_path.exists() else {'epoch':0,'cu':0}
            delay=max(0.5,float(last['cu'])/200)-(self.clock()-last['epoch'])
            if delay>0:self.sleep(delay)
            atomic_json(last_path,{'epoch':self.clock(),'cu':getattr(self,'_dispatch_cu',0),
                                  'policy':'RECOVERY_MAX_2_HTTP_PER_SECOND_AND_200_CU_PER_SECOND'})
            with http_requests.Session() as session:
                # requests' adapter defaults to zero retries; explicitly retain it.
                session.mount('https://',http_requests.adapters.HTTPAdapter(max_retries=0))
                with session.post('https://eth-mainnet.g.alchemy.com/v2/'+key,data=canonical(requests),
                     headers={'Content-Type':'application/json','Accept-Encoding':'identity'},timeout=(10,60),stream=True,allow_redirects=False) as response:
                    # Bound decoded JSON too if a provider ignores identity.
                    return response.status_code,response.raw.read(RPC_MAX+1,decode_content=True),{'Retry-After':response.headers.get('Retry-After')}
    ...
    def call_batch(self, plans, probe, label, capability=False, *, deadline=None):
        ...
        if recovery:
            groups=[]; current=[]
            for plan in plans:
                if plan.get('method')=='eth_getTransactionReceipt':
                    if current:groups.append(current);current=[]
                    groups.append([plan])
                else:
                    current.append(plan)
                    if len(current)==5:groups.append(current);current=[]
            if current:groups.append(current)
            if len(groups)>1:
                results=[self.call_batch(group,probe,label,capability,deadline=deadline) for group in groups]
                return {'status':'COMPLETE' if all(r['status']=='COMPLETE' for r in results) else 'PARTIAL',
                    'members':[member for r in results for member in r['members']],
                    'attempt_batches':sum(r['attempt_batches'] for r in results),
                    'actual_operations_this_call':sum(r['actual_operations_this_call'] for r in results),
                    'snapshot':results[-1]['snapshot'],'ordinary_batch_limit':5,'receipt_batch_limit':1}
        active_validate_rpc = getattr(self.runtime, 'validate_rpc', validate_rpc)
        active_rpc_identity = getattr(self.runtime, 'rpc_identity', rpc_identity)
        active_rpc_result_status = getattr(self.runtime, 'rpc_result_status', rpc_result_status)
        plans = [active_validate_rpc(p) for p in plans]
        provider = 'ALCHEMY_ETH_MAINNET_EXISTING' if self.transport is None else 'SYNTHETIC_TRANSPORT'
        identities = [active_rpc_identity(provider, p) for p in plans]
        if len({logical_key(k) for k in identities}) != len(plans): raise ValueError('Duplicate batch member')
        permission = self.permission()
        if getattr(self.runtime,'rpc_permission',None):permission=self.runtime.rpc_permission(permission)
        ...
        active_session = self.runtime.session if self.runtime is not None else session
        active_raw_risk = self.runtime.raw_risk if self.runtime is not None else raw_risk
        with active_session(self.w, probe, label, clock=self.clock, monotonic=self.monotonic, synthetic=self.transport is not None) as timing:
            ...
            while pending:
                ...
                for index in pending:
                    ...
                    if claim['state'] == 'CACHE_HIT':
                        ...
                        if active_rpc_result_status is not rpc_result_status:
                            cached_env = read(cached_path)
                            cached_request = cached_env.get('request', {})
                            cached_response = cached_env.get('response', {})
                            if ({k: cached_request.get(k) for k in ('method', 'params')} != plans[index]
                                or active_rpc_result_status(cached_request, cached_response) != 'SUCCESS_VALIDATED'
                                or cached_response.get('result') != claim['payload']):
                                raise RuntimeError('Successful RPC cache no longer validates against runtime scope')
                ...
                if recovery and len(claimed)>1 and any(c['attempt_no']>=3 for _,c in claimed):
                    for _,extra in claimed[1:]:self.store.abandon_before_dispatch(extra['attempt_id'],'RECOVERY_BATCH_TWO_FAILURES_USE_SINGLE')
                    claimed=claimed[:1]
                if not claimed:
                    ...
                    break
                raw_cap = self.runtime.raw_limit(self.w) if getattr(self.runtime, 'raw_limit', None) else RAW_CAP
                if active_raw_risk(self.w) + RPC_MAX + 1 > raw_cap:
                ...
                reserved = []; raw_reservation_created = False
                try:
                    if capability:
                        with self.ledger.connection() as db:
                        ...
                        old_keys = {logical_key(active_rpc_identity(provider, json.loads(r[0]))) for r in old_plans}
                    ...
                    if getattr(self.runtime, 'reserve_raw', None):
                        self.runtime.reserve_raw(self.w, batch, 3*RPC_MAX+65536)
                        raw_reservation_created = True
                    ...
                    if raw_reservation_created:
                        closing_receipt = directory / 'setup_failure_receipt.json'
                        new_json(closing_receipt, {'batch': batch, 'http_transport_called': False,
                            'reason': 'LOCAL_DISPATCH_SETUP_FAILED_BEFORE_TRANSPORT', 'utc': now()})
                        self.runtime.close_raw(self.w, batch, closing_receipt)
                    raise
                self._dispatch_cu=sum(rates[plans[index]['method']] for index,_,_ in reserved)
                ...
                for index, claim, job in reserved:
                    ...
                    if member_failure is None:
                        ...
                        elif active_rpc_result_status(request, response) != 'SUCCESS_VALIDATED': member_failure = classify_failure(category=active_rpc_result_status(request, response))
                ...
                if getattr(self.runtime, 'close_raw', None): self.runtime.close_raw(self.w, batch, directory / 'receipt.json')
