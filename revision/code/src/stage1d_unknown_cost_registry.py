...
class Registry:
    ...
    def _load_role_binding(self):
        ...
        for name in ('CURRENT.json', 'AUTHORITIES.json', 'USER_TASK_BOUNDARIES.json'):
            ...
            ref = self._ref(path)
            if name != 'USER_TASK_BOUNDARIES.json':
                visit(ref)
                continue
            # TaskBoundaries consumes the inline user_message/SHA and frozen
            # applicability. Its pre_adoption_collections are historical audit
            # references to mutable aliases, not inputs to current role checks.
            # Bind them as original text inside this top-level document, without
            # re-reading those aliases or making role identity depend on replay.
            self.raw(ref); refs[(ref['path'], ref['sha256'])] = deepcopy(ref)
            document = self.read(ref)
            for field, value in document.items():
                if field != 'boundaries': visit(value)
            for record in document.get('boundaries', []):
                for field, value in record.items():
                    if field == 'pre_adoption_collections': continue
                    if (field == 'authority_request_ref' and isinstance(value, dict)
                            and str(value.get('path', '')).startswith('operations/')):
                        # This explicit field in the adopted user-hold document
                        # names the revision-level original operation receipt.
                        # No generic path fallback or historical raw exemption.
                        original = (self.root/value['path']).resolve()
                        if not original.is_relative_to(self.root/'operations'):
                            raise ValueError('User authority request escapes revision operations')
                        import os
                        bound = dict(value, path=Path(os.path.relpath(original, self.work)).as_posix())
                        visit(bound)
                    else:
                        visit(value)
