...
from stage1d_multiasset_context import (SCHEMA as MULTIASSET_SCHEMA, ETH, WETH, ASSETS,
    key_asset, boundary, ordered_operations, validate_extension)
CONTEXT_SCHEMA = "stage1b-r3-context-model-v1"
CONTEXT_SCHEMAS = {CONTEXT_SCHEMA, MULTIASSET_SCHEMA}
...
def observed_targets(document):
    ...
    """
    if document.get("schema_version") in CONTEXT_SCHEMAS:
...
def _compile_context(document):
    ...
    if multiasset: validate_extension(document)
    ...
    if len(accounts) != len(document["accounts"]) or any(k.startswith("@outside_reservoir|") for k in accounts):
    ...
    for key, account in accounts.items():
        if key_asset(key) not in (ASSETS if multiasset else {ETH}):
    ...
    initial.update({boundary(a): F(0) for a in (ASSETS if multiasset else {ETH})})
    anchors = {}
    ...
    for tx in document["transactions"]:
        ...
        for fee in tx.get("fees", []):
            ...
            if fee.get("timing") != "TX_BEGIN_NET_FEE" or payer in terminals or key_asset(payer) != ETH:
        ...
        for flow in ordered_operations(tx):
            if flow.get("kind") == "conversion":
                if not multiasset:
                    raise ValueError("Native context cannot carry conversions")
                sender, receiver = flow["from_account"], flow["to_account"]
                if sender not in accounts or receiver not in accounts:
                    raise ValueError("Conversion requires both actual holder ledgers")
                operations.append({"kind": "conversion", "id": flow["id"], "asset": flow["asset"],
                    "output_asset": flow["output_asset"], "sender": sender, "receiver": receiver,
                    "refund_to": sender, "gross": _raw(flow["gross_raw"]),
                    "refund": _raw(flow.get("refund_raw", "0")), "output": _raw(flow["output_raw"])})
                continue
            role, sender, receiver = flow["role"], flow.get("from_account"), flow.get("to_account")
            flow_asset = flow.get("asset", "ETH")
            if flow_asset not in (ASSETS if multiasset else {ETH}) or any(k is not None and key_asset(k) != flow_asset for k in (sender, receiver)):
                raise ValueError("Context flow asset identity mismatch")
            pool = boundary(flow_asset)
            ...
            operations.append(_flow(flow["event_id"], sender, receiver, flow_asset, flow["amount_raw"], role, fixed))
        emit_anchors("post")
...
def _compile(document):
    if 'transactions' in document and document.get('schema_version') not in CONTEXT_SCHEMAS:
        raise ValueError('Unregistered context schema')
    initial, operations, targets, context = _compile_context(document) if document.get("schema_version") in CONTEXT_SCHEMAS else _compile_graph(document)
...
def _boundary_completion(operations, context):
    assets = {op.get("asset") for op in operations if op.get("asset")} | {op["output_asset"] for op in operations if op.get("output_asset")}
    if context and WETH in assets:
        by_asset, amounts = {}, {}
        for a in sorted(assets):
            incoming = outgoing = minimum = F(0)
            sequence = []
            for op in operations:
                if op["kind"] != "flow" or op["asset"] != a:
                    continue
                pool = boundary(a)
                if op["sender"] == pool: incoming += op["amount"]
                elif op["receiver"] == pool: outgoing += op["amount"]
                else: continue
                minimum = max(minimum, incoming - outgoing)
                sequence.append({"event_id": op["id"], "direction": "RETURN" if op["sender"] == pool else "EXIT",
            ...
            by_asset[a] = {"convention": "H_BMIN_BOUNDARY_V1", "adopted": bool(incoming), "is_observed_fact": False,
                "B0_out_raw": _fmt(minimum), "S0_out_raw": "0", "asset": a,
                "selection_basis": "max(0, max_prefix(cumulative_unknown_returns - cumulative_nonservice_exits))", "sequence": sequence}
            amounts[boundary(a)] = minimum
        result = dict(by_asset[ETH])
        result.update(adopted=any(x["adopted"] for x in by_asset.values()), by_asset=by_asset,
            amount_scope="SEPARATE_ASSET_POOLS_NO_CROSS_ASSET_SUM")
