...
def _certify(model: Model, result, cost: list[F]):
    ...
    certificate = {"certified": passed, "exact_primal_feasible": feasibility["exact_feasible"],
            ...
            "failure_detail": None if passed else feasibility}
    # A boolean is insufficient for receiver-side subset inference. Keep the
    # exact maximum-zero dual witness; a later context primal recovery may
    # certify this same dual, but derivation still requires a feasible witness.
    if certificate["exact_dual_feasible"] and dual == 0 and any(c < 0 for c in cost) and all(c <= 0 for c in cost):
        certificate["exact_zero_upper_dual"] = {"schema": "exact-zero-upper-dual-v1",
            "equality_multipliers": [fmt(v) for v in eqdual],
            "lower_multipliers": [fmt(v) for v in lowerdual],
            "upper_multipliers": [fmt(v) for v in upperdual]}
    return certificate, vector, primal
...
def solve_interval(model: Model, event_names: list[str], time_limit_seconds=30) -> dict:
    ...
    if not all(math.isfinite(v) for pair in bounds for v in pair) or not all(math.isfinite(v) for v in vv):
        return {"status": "NUMERICAL_UNKNOWN", "lower_raw": None, "upper_raw": None, "reason": "nonfinite_double_representation", "solver_call_count": 0}
    endpoints = {}
    ...
    return {"status": "OPTIMAL_EXACT_CERTIFIED" if good else "UNRESOLVED",
            ...
            "endpoints": endpoints, "solver_call_count": len(endpoints)}
