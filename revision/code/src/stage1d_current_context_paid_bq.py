...
class _Points:
    def __init__(self,work,bindings,*,reuse_success_cache=False,allow_paid_top_fields=False):
        self.work=work;self.members={};self.values={};self.used=[]
        self.reuse_success_cache=reuse_success_cache
        self.allow_paid_top_fields=allow_paid_top_fields
        self.checked_missing=set()
    ...
    def get(self,plan):
        key=h.digest(plan)
        if key not in self.members and self.reuse_success_cache and key not in self.checked_missing:
            member=transfers.cached_member(self.work,plan)
            if member is None:self.checked_missing.add(key)
            else:self.members[key]={'plan':deepcopy(plan),'member':member}
...
def _bind_selected(families,points,evidence):
    ...
    for tx,family in families.items():
        ...
        paid_eligible=False
        if points.allow_paid_top_fields:
            try:paid_eligible=paid_top_fields(family) is not None
            except (ValueError,KeyError,TypeError,IndexError) as error:
                result[tx]=family;gaps.append({'reason':'PAID_TOP_PHYSICAL_FIELD_CONFLICT','tx_hash':tx,
                    'block_number':block,'detail':str(error),'needed_rpc':[],
                    'provided_binding_evidence_failed':True});continue
        gap={'reason':'ROOT_EQUIVALENCE_UNRESOLVED_GAP','tx_hash':tx,'block_number':block,
             'data_type':h.INTERNAL,'needed_rpc':([plans[2]] if values[2] is None else []) if paid_eligible else missing}
        if paid_eligible:
            gap['ordinary_top_fields_available_from_verified_paid_source']=True
            gap['full_receipt_or_token_logs_claimed']=False
        if not missing or paid_eligible and values[2] is not None:
            try:
                ids=evidence+['sha256:'+points.members[h.digest(p)]['member']['artifact_sha256']
                              for p,v in zip(plans,values) if v is not None]
                bound=(_bind(traces,*values,ids) if not missing else bind_paid_top_header(family,values[2],ids))
                if bound.get('paid_top_field_proof'):
                    bound['root_binding']['paid_top_field_proof']=bound['paid_top_field_proof']
                tops=[r for r in family if r['record_type']=='transaction']
                ...
                bindings.append({'tx_hash':tx,'root_binding':bound['root_binding'],
                                 'point_request_keys':[h.digest(p) for p,v in zip(plans,values) if v is not None]})
                continue
            except (ValueError,KeyError,TypeError,IndexError) as error:
...
def _admit_plan(scope,plan,sources,points):
    ...
    material=merge_bound_rows([],material)
...
def admit_current(work,query_name,collection_ref,label_ref,source_inventory,*,point_bindings=(),reuse_success_cache=False):
    ...
    points=_Points(work,point_bindings,reuse_success_cache=reuse_success_cache,allow_paid_top_fields=True)
    ...
    result.update(schema_version=SCHEMA,query_id=query['query_id'],scope_hash=query['scope_hash'],
        ...
        current_semantic_units=len(collection.get('semantic_units',[])),
        ordinary_paid_top_field_binding_enabled=True,
        evidence_trust='ORIGINAL_SQL_SCHEMA_DRY_JOB_RAW_PAGE_AND_EXACT_MEMBER_REVERIFICATION')
