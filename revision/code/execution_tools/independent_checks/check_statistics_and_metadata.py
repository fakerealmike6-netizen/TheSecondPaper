...
index=batch_index();stats=read(REPORTS/'STATISTICS.json');bykind=defaultdict(list);count=Counter();cgroups=Counter();halfn=Counter();recalls=defaultdict(list);timing_n=0
for row in index['method_results_index']:
 p=batch_path(row['path']);m=read(p/'METHOD_RESULTS.json');ev=read(p/'EVALUATION.json');ab=read(p/'ABLATIONS.json');eff=read(p/'EFFICIENCY.json')
...
tiny=[read(batch_path(row['path'])/'EVALUATION.json')['tiny_crosscheck'] for row,_,_,_,_ in bykind['controlled'] if read(batch_path(row['path'])/'EVALUATION.json')['tiny_crosscheck'].get('status')!='NOT_REQUIRED']
...
# Compare against the supplied, accepted Stage1C MIN extraction, not an
# unavailable external absolute Stage1B handoff path. This checks the immediate
# accepted parent; it makes no new claim of reopening the original Stage1B DB.
prior_budget=BASELINE/'private/ledger/shared_budget_r4.sqlite'
prior_db_present=prior_budget.is_file()
assert prior_db_present and sha(prior_budget)==before
for n in ['private/FINAL_BUDGET_SNAPSHOT_R4.json','private/CUMULATIVE_REQUEST_COST_ROWS_R4.json']:
 assert read(ROOT/n)==read(BASELINE/n)
