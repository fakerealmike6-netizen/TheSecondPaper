...
def main():
 ...
 for row in manifest['samples']:
  ...
  f=sample_result(row['sample_id'],'controlled')/'METHOD_RESULTS.json';results=read(f);ev=read(f.parent/'EVALUATION.json')
  details={}
  for variant in ('FULL_INTERVAL','BALANCE_INFORMATION_REMOVED','NO_PROTOCOL_CONTINUATION','NO_CROSS_TARGET_COUPLING'):
   ...
   assert set(value['joint_by_asset'])==({k.rsplit('|',1)[1] for k in g['objective_groups']} or {next(e['asset'] for e in g['events'] if e['kind']=='seed')})
 ...
 (OUT/'independent_controlled_results.json').write_text(json.dumps(report,indent=2));print({k:v for k,v in report.items() if k!='results'})
