...
class Stage1CRunnerTests(unittest.TestCase):
    ...
    def test_protocol_absence_is_an_explicit_identical_interval_control(self):
        ...
        self.assertEqual(set(full['joint_by_asset']),set(ablated['joint_by_asset']))
        for asset in full['joint_by_asset']:
            left=full['joint_by_asset'][asset];right=ablated['joint_by_asset'][asset]
            # Same mathematical domain/endpoints; certificate identities must
            # still bind their own method and cannot be reused across variants.
            self.assertEqual(left['proof_model_identity']['method'],'FULL_INTERVAL')
            self.assertEqual(right['proof_model_identity']['method'],'NO_PROTOCOL_CONTINUATION')
            self.assertEqual({k:v for k,v in left.items() if k!='proof_model_identity'},
                             {k:v for k,v in right.items() if k!='proof_model_identity'})
